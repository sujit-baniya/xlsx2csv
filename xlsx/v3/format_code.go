...
func (fullFormat *parsedNumberFormat) formatNumericCell(cell *Cell) (string, error) {
	...
	if containsDateChars(fullFormat.numFmt) {
	    f, _ := strconv.ParseFloat(rawValue, 64)
        time := TimeFromExcelTime(f, false)
...
func parseNumberFormatSection(fullFormat string) (*formatOptions, error) {
	...
	if len(remaining) > 0 {
		...
		log.Print(errors.New("invalid or unsupported format string"))
	}
...
func containsDateChars(format string) bool {
	found := false
	for _, special := range timeFormatCharacters {
		if strings.HasPrefix(format, special) {
			found = true
			break
		}
	}
